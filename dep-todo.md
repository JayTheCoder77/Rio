...
**Before Phase 4**: commit the four Dockerfiles (`apps/github-app/Dockerfile`
replaced, `apps/worker/Dockerfile` + `apps/worker/src/serve.ts` new,
`services/ai-engine/Dockerfile` new, `services/sandbox-runner/Dockerfile`
fixed) and a root `.dockerignore`, and push to `main`. All four Render
services in Phase 4 deploy via Docker with **build context = repo root** —
this is called out again at each service below since it's the setting most
...
- [x] GitHub account (you already have this — used for App/OAuth registration)
- [x] Neon account (neon.tech) — Postgres
- [x] Upstash account (upstash.com) — Redis
- [x] Pinecone account (pinecone.io) — vector index
- [x] Render account (render.com) — no card required for free tier
- [x] Vercel account (vercel.com) — for `apps/web`
- [x] PyPI account (pypi.org) — only needed later, for publishing the CLI
- [x] Free account with **either** Groq (console.groq.com) or OpenRouter
  ```
...
- [x] Create a new Neon project (any region close to where Render will run —
  ```
  pick the same region for both to cut latency).
  ```
- [x] Copy the pooled connection string. This is your `DATABASE_URL`.
- [x] Save it somewhere temporary (password manager / local `.env`, not committed).



### 1.2 Upstash (Redis)

- [x] Create a new Redis database, **regional** (not global — BullMQ doesn't
  ```
  benefit from multi-region and it complicates consistency).
  ```
- [x] Copy the **TLS** connection string (`rediss://...`). This is your `REDIS_URL`.



### 1.3 Pinecone

- [x] Create a Starter-plan project.
- [x] Create a serverless index named `rio` (matches the code's default
  ```
  ...
  ```
- [x] Copy the API key. This is `PINECONE_API_KEY`.



### 1.4 Generate shared secrets (do this once, locally)

- [x] `openssl rand -base64 32` → this is `INTERNAL_SERVICE_TOKEN`. Must be
  ```
  **identical** on `apps/worker` and `services/ai-engine` — this is the
  shared secret the worker uses to authenticate to ai-engine.
  ```
- [x] `openssl rand -hex 32` → this is `ENCRYPTION_KEY`. Must be **identical**
  ```
...
- [x] Locally, in the repo root, create a `.env` (not committed) with at least
  ```
  `DATABASE_URL=<your Neon string>`.
  ```
- [x] `bun install` at repo root (installs `drizzle-kit` as part of
  ```
  `packages/db`'s devDependencies).
  ```
- [x] `cd packages/db && bunx drizzle-kit migrate` — applies all 9 existing
  ```
  migrations (`0000` through `0008`) to the fresh Neon database.
  ```
- [x] Verify in the Neon console's table browser that tables like `users`,
  ```
...
- [x] Name it (e.g. `rio-code-review-yourname` — must be globally unique).
- [x] Homepage URL: your future Vercel URL (placeholder is fine for now, e.g.
  ```
  `https://rio.vercel.app`).
  ```
- [x] Webhook URL: placeholder for now (`https://example.com`) — you'll edit
  ```
  this after Phase 4.2.
  ```
- [x] Generate and note a **webhook secret** (`WEBHOOK_SECRET`) — anything
  ```
  random, doesn't need to match anything else.
  ```
- [x] Permissions: check `apps/github-app/src` for the exact scopes it uses
  ```
  (pull requests, checks, contents at minimum for a review bot) and grant
  only those.
  ```
- [x] Subscribe to the webhook events the app actually handles (check
  ```
  `apps/github-app/src/index.ts` for the event names it listens to).
  ```
- [x] Create the app, then generate a **private key** (`.pem` file) —
  ```
  download it, keep it safe.
  ```
- [x] Note the **App ID** shown on the app's settings page.



### 3.2 GitHub OAuth App (github.com/settings/developers → OAuth Apps)

- [x] This is separate from the GitHub App above — it's for the "Sign in with
  ```
  GitHub" flow on `apps/web` (NextAuth).
  ```
- [x] Homepage URL: your future Vercel URL.
- [x] Authorization callback URL: `https://<your-vercel-domain>/api/auth/callback/github`
  ```
  (placeholder domain for now, fix after Phase 6).
  ```
- [x] Note the **Client ID** and generate a **Client Secret**.
...
All four services below deploy as **Docker**, all four Dockerfiles use the
**repo root as build context** (to reach `packages/rio-core`, `packages/db`,
etc. — see the CI job's `docker build -f services/sandbox-runner/Dockerfile .`
for the pattern). For every one of them, on Render:

- **New → Web Service → connect the GitHub repo**
- Runtime: **Docker**
- Dockerfile Path: as noted per service below
- **Docker Build Context Directory:** `.` (repo root) — this is the field
that actually matters here. It defaults to the service's own subdirectory,
which will fail to find the workspace-shared packages. Set it explicitly
every time, for all four services.
- Root Directory can stay unset/`.` for the same reason — the Dockerfile
itself is what does the "pick out only what this service needs" work.



### 4.1 `services/sandbox-runner` (deploy first — no dependencies)

- [x] Dockerfile Path: `services/sandbox-runner/Dockerfile`
- [x] Docker Build Context Directory: `.`
- [x] No environment variables needed (it's stateless — takes code, runs
  ```
  ruff/mypy/semgrep/eslint/tsc, returns results).
  ```
- [x] Deploy, confirm `/` or its health endpoint responds.
- [x] Note its URL (e.g. `rio-sandbox-runner.onrender.com`).



### 4.2 `services/ai-engine`

- [x] Dockerfile Path: `services/ai-engine/Dockerfile`
- [x] Docker Build Context Directory: `.`
...
- [x] Deploy, confirm `GET /v1/health` returns `{"status": "ok"}`.
- [x] Note its URL (e.g. `rio-ai-engine.onrender.com`) — **you'll need this
  ```
...
- [x] **Use the replaced Dockerfile**, not the original one from the repo —
  ```
  the original assumed npm/`package-lock.json`, which don't exist in this
  bun monorepo, and would fail to build.
  ```
- [x] Dockerfile Path: `apps/github-app/Dockerfile`
- [x] Docker Build Context Directory: `.`
- [x] Environment variables:
  - `APP_ID` = Phase 3.1
  ...
  - `LOG_LEVEL` = `info`
- [x] Deploy, confirm the service is up (Probot logs a startup line if the
  ```
  app booted and connected correctly — check Render's log tab).
  ```
- [x] Note its URL (e.g. `rio-github-app.onrender.com`).
- [x] **Go back to Phase 3.1** and update the GitHub App's Webhook URL to
  ```
...
No HTTP server exists here natively, and Render's free tier only covers Web
Services — solved via `apps/worker/src/serve.ts` (a tiny Bun health server
that then `import`s the real consumer) plus the worker Dockerfile, both
already written. Deployed as **two** Render services from the same
Dockerfile/image, split by an env var.

- [x] Confirm `apps/worker/src/serve.ts` exists (health check + dynamic
  ```
  import of `worker.ts` or `indexWorker.ts` based on `WORKER_ENTRY`).
  ```
- [x] Dockerfile Path: `apps/worker/Dockerfile`
- [x] Docker Build Context Directory: `.`
- [x] Deploy **service 1** ("review worker"):
  - Environment variables:
    ...
    - `WORKER_ENTRY` = `review` (or leave unset — `review` is the default)
- [x] Deploy **service 2** ("index worker") — same Dockerfile, same env vars,
  ```
  except:
  ```
  - `WORKER_ENTRY` = `index`
- [x] For both: confirm the health endpoint (`/healthz`) responds and logs
  ```
  show a Redis connection established (BullMQ/ioredis logs this on
  connect).
  ```
...
- [x] Sign up at cron-job.org (or UptimeRobot) — free.
- [x] Add a job pinging `https://rio-github-app.onrender.com/healthz` (or
  ```
  whatever health path it exposes) every 10 minutes.
  ```
- [x] Add a second job pinging the worker's health endpoint every 10 minutes.
- [x] Optional: ping ai-engine's `/v1/health` too if you want the CLI/dashboard
  ```
...
- [x] Vercel → New Project → import the repo.
- [x] Root Directory: `apps/web`.
- [x] Framework preset: Next.js (auto-detected).
- [x] Environment variables — check `apps/web/auth.config.ts` and
  ```
...
- [x] Deploy. Note the real Vercel URL (e.g. `rio-yourname.vercel.app`).
- [x] **Go back to Phase 3.2** and fix the OAuth App's callback URL and
  ```
  homepage to use this real URL instead of the placeholder.
  ```
- [x] **Go back to Phase 3.1** and fix the GitHub App's homepage URL too.
- [x] Sign in via "Sign in with GitHub" on the deployed site and confirm a row
  ```
...
- [x] In `apps/cli/rio_cli/config.py`, change the default so it's overridable
  ```
...
- [x] Bump the version in `apps/cli/pyproject.toml`.
- [x] `cd apps/cli && uv build` (or `python -m build`) to produce the wheel/sdist.
- [x] `twine upload dist/*` to publish to PyPI (needs the PyPI account +
  ```
  an API token from Phase 0).
  ```
- [x] In a fresh shell/venv: `pip install rio-cli`, then `rio auth` — generate
  ```
...
- [x] On the deployed web dashboard, go to Settings → connect a Groq or
  ```
  OpenRouter key (from Phase 0), pick a model.
  ```
- [x] **CLI path**: in a local git repo with some staged changes, run
  ```
  ...
  ```
- [x] **GitHub App path**: install the GitHub App on a real repo you control,
  ```
...
- [x] Web dashboard reachable, GitHub sign-in works
- [x] GitHub App installable on a repo, webhook delivers successfully
- [x] A PR triggers a posted review comment end to end
- [x] `pip install rio-cli` + `rio auth` + `rio review --staged` works from a
  ```
  clean environment with no local config
  ```
- [x] BYOK settings page stores and later successfully decrypts a provider key
- [ ] All five services auto-redeploy on push to `main`
- [x] Keep-alive pings are running so cold starts aren't the first thing a
  ```
  demo viewer hits
  ```
